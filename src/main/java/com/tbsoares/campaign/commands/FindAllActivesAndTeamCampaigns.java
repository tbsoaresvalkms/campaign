...
@Component
public class FindAllActivesAndTeamCampaigns {
    private final CampaignRepository campaignRepository;
    private final Converter converter;

    public FindAllActivesAndTeamCampaigns(CampaignRepository campaignRepository, Converter converter) {
        this.campaignRepository = campaignRepository;
        this.converter = converter;
    }

    public List<CampaignResource> execute(String team) {
        team = team.toUpperCase();
        return campaignRepository.findAllByTeamAndEndDateGreaterThanEqual(team, LocalDate.now())
                .stream()
                .map(this::toResource)
